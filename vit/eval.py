...
def eval_single_dataset_with_loss(image_encoder, dataset_name, args):
    classification_head = get_classification_head(args, dataset_name)
    model = ImageClassifier(image_encoder, classification_head)
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(tqdm.tqdm(dataloader)):
            ...
            logits = utils.get_logits(x, model)
            ce_loss += torch.nn.functional.cross_entropy(logits, y, reduction="sum")
            pred = logits.argmax(dim=1, keepdim=True).to(device)
    ...
    return metrics, ce_loss.item()

def eval_single_dataset_return_loss(image_encoder, dataset_name, args):
    classification_head = get_classification_head(args, dataset_name)
    model = ImageClassifier(image_encoder, classification_head)
    ...
    with torch.no_grad():        
        for i, data in enumerate(dataloader):
            ...
            ce_loss += torch.nn.functional.cross_entropy(logits, y, reduction="sum")
            n += y.size(0)
    return ce_loss.item(), n


def eval_single_dataset_return_predict(image_encoder, dataset_name, args):
    classification_head = get_classification_head(args, dataset_name)
    model = ImageClassifier(image_encoder, classification_head)
    ...
    metrics = {'top1': top1}
    print(f'Done evaluating on {dataset_name}. Accuracy: {100*top1:.2f}%')
    print("np.concatenate(predict) = ", np.concatenate(predict).shape)
    return metrics, np.concatenate(predict)
...
def eval_single_dataset_preprocess_head(image_encoder, head, dataset_name, args):
    ...
    dataset = get_dataset(dataset_name, model.val_preprocess, location=args.data_location,  batch_size=args.batch_size, num_workers=64)
    dataloader = get_dataloader(dataset, is_train=False, args=args, image_encoder=None)
    device = args.device
